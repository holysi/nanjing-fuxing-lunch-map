const state = { places: [], filtered: [], map: null, markers: [], activeId: null, view: "map" };
const $ = (selector) => document.querySelector(selector);
const categoryClass = { "健康餐": "health", "日式": "japanese", "中式": "chinese" };
const categoryEmoji = { "健康餐": "🥗", "日式": "🍱", "中式": "🥟" };
...
function renderMarkers() {
  ...
  state.markers = state.filtered.filter((place) => Array.isArray(place.coordinates) && place.coordinates.length === 2).map((place) => {
    ...
    const marker = L.marker(place.coordinates, { icon }).addTo(state.map);
    const popup = document.createElement("div");
    popup.innerHTML = `<strong>${escapeHtml(place.name)}</strong><br><span>${escapeHtml(priceLabel(place))} · ${escapeHtml(place.category)}</span><br><button class="popup-detail" type="button">查看詳情 →</button>`;
...
async function start() {
  ...
  try {
    ...
    $("#hero-count").textContent = `已收錄 ${state.places.length} 間午餐店`;
    applyFilters();
    initMap();
